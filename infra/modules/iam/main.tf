...
resource "aws_iam_policy" "eso_secrets_policy" {
  ...
  description = "Allow External Secrets Operator to read tomato secrets from AWS Secrets Manager"
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:secretsmanager:*:${var.aws_account_id}:secret:/tomato/*"
      }
    ]
  })
}
